...
-- 1. Bersihkan Data/Tabel Lama
DROP TABLE IF EXISTS Transaksi;
DROP TABLE IF EXISTS Barang;
-- (Jika gagal saat drop Barang, pastikan tabel Transaksinya sudah terdrop lebih dulu)

-- 2. Buat Ulang Tabel Barang (Memakai ID Cabang)
CREATE TABLE Barang (
    KodeBarang VARCHAR(50),
    ID_Cabang INT FOREIGN KEY REFERENCES Cabang(ID_Cabang),
    NamaBarang VARCHAR(100),
    Kategori VARCHAR(50),
    Harga DECIMAL(18,2),
    Stok INT,
    -- Memaksimalkan stok per cabang
    PRIMARY KEY(KodeBarang, ID_Cabang)
);
GO

-- 3. Buat Ulang Tabel Transaksi
CREATE TABLE Transaksi (
    ID_Transaksi INT IDENTITY(1,1) PRIMARY KEY,
    ID_Cabang INT FOREIGN KEY REFERENCES Cabang(ID_Cabang),
    ...
    TanggalTransaksi DATETIME DEFAULT GETDATE(),
    -- Ikut ke constraint 2 kolom
    FOREIGN KEY(KodeBarang, ID_Cabang) REFERENCES Barang(KodeBarang, ID_Cabang)
);
GO

-- 4. Sampel Data Barang (Hanya agar tidak kosong total)
INSERT INTO Barang (KodeBarang, ID_Cabang, NamaBarang, Kategori, Harga, Stok) 
VALUES ('BRG001', 1, 'Kemeja Polos', 'Pakaian', 150000, 50);
GO
